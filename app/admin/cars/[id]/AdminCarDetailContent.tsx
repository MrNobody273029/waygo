  ...
  return (
    <div className="space-y-6 max-w-screen-lg">
      ...
      <div className="flex items-center gap-2 text-sm text-slate-500">
        <Link href="/admin/cars" className="hover:text-slate-900 transition">Cars</Link>
        <span>/</span>
      ...
      <div className="rounded-3xl border bg-white shadow-soft p-6">
        <div className="flex flex-col sm:flex-row sm:items-start gap-5">
          ...
          >
            {toggling ? '…' : currentActive ? 'Deactivate' : 'Activate'}
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 pt-5 border-t">
          {[
            { label: 'Daily price', value: `${gel(car.dailyPrice)}/day` },
            { label: 'Total revenue', value: gel(totalRevenue) },
            { label: 'Completed trips', value: completedBookings.toString() },
            { label: 'Active now', value: activeBookings.toString() },
          ].map(s => (
            <div key={s.label}>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_300px]">
        <div className="space-y-5">
          ...
          {car.imageUrls.length > 1 && (
            <div className="rounded-2xl border bg-white shadow-soft p-5">
              <h3 className="font-black text-base mb-3">All Photos</h3>
          ...
          <div className="rounded-2xl border bg-white shadow-soft overflow-hidden">
            <div className="border-b px-5 py-4 flex items-center justify-between">
              <h3 className="font-black text-base">Booking History</h3>
            ...
            {car.bookings.length === 0 ? (
              <p className="px-5 py-6 text-sm text-slate-400">No bookings yet.</p>
            ) : (
                  ...
                  <tbody className="divide-y">
                    {car.bookings.map(b => (
                      <tr key={b.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-5 py-3.5">
                          <Link href={`/admin/bookings/${b.id}`} className="text-xs font-bold text-primary hover:underline">View →</Link>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div className="space-y-5">
          {/* Specs */}
          <div className="rounded-2xl border bg-white shadow-soft p-5">
            <h3 className="font-black text-base mb-4">Specifications</h3>
            <div className="space-y-2 text-sm">
              {[
                ...
                { label: 'Steering', value: car.steeringWheel },
                { label: 'Min days', value: car.minDays.toString() },
                { label: 'Notice', value: `${car.advanceNotice}h` },
                ...(car.estimatedValueUsd ? [{ label: 'Est. value', value: `$${car.estimatedValueUsd.toLocaleString()}` }] : []),
          ...
          <div className="rounded-2xl border bg-white shadow-soft p-5">
            ...
            <div className="space-y-1.5 text-xs text-slate-600">
              <div className="flex justify-between"><span>Phone</span><span className="font-semibold">{car.owner.phone || '—'}</span></div>
              <div className="flex justify-between"><span>Country</span><span className="font-semibold">{car.owner.country}</span></div>
              <div className="flex justify-between"><span>Host status</span>
