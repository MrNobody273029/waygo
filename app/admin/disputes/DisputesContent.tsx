...
// STATUS_LABEL is now computed inside components using useLang()
...
function PhotoStrip({ photos, label }: { photos: string[]; label: string }) {
  ...
  if (photos.length === 0) return (
    <div>
      ...
      <p className="text-label-sm text-slate-400 italic">{t.admin.disputeNoPhotos}</p>
    </div>
  );
  return (
    <div>
...
function DisputeCard({ dispute, onRefresh }: { dispute: DisputeItem; onRefresh: () => void }) {
  ...
  const STATUS_LABEL: Record<string, string> = {
    PENDING:         t.admin.disputeStatusPending,
    EXPERT_ASSIGNED: t.admin.disputeStatusExpert,
    RESOLVED:        t.admin.disputeStatusResolved,
  };
      ...
      {expanded && (
        <div className="border-t border-outline-variant/30 px-5 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-label-sm">
            <div>
              <p className="text-secondary mb-0.5">{t.admin.disputeBookingId}</p>
              <p className="font-semibold text-on-background font-mono text-[11px]">{dispute.booking.id.slice(0, 8).toUpperCase()}</p>
            </div>
            <div>
              <p className="text-secondary mb-0.5">{t.admin.guest}</p>
            ...
            <div>
              <p className="text-secondary mb-0.5">{t.admin.owner}</p>
            ...
            <div>
              <p className="text-secondary mb-0.5">{t.admin.disputeOpened}</p>
          ...
          <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3">
            <p className="text-[11px] font-bold text-amber-700 uppercase tracking-wider mb-1">{t.admin.disputeHostComment}</p>
            <p className="text-label-sm text-amber-900 leading-relaxed">{dispute.hostComment}</p>
          </div>

          {/* Photos grid */}
          <div className="grid sm:grid-cols-3 gap-4">
            <PhotoStrip photos={dispute.booking.pickupPhotos} label={t.admin.disputePickupPhotos} />
            <PhotoStrip photos={dispute.booking.returnPhotos} label={t.admin.disputeReturnPhotos} />
            <PhotoStrip photos={dispute.hostPhotos} label={t.admin.disputeHostPhotos} />
          </div>

          {/* Resolution info (if resolved) */}
          {dispute.status === 'RESOLVED' && (
            <div className="rounded-xl bg-tertiary-fixed/20 border border-tertiary/20 px-4 py-3 space-y-1">
              <p className="text-[11px] font-bold text-tertiary uppercase tracking-wider">{t.admin.disputeResolution}</p>
              <div className="flex gap-4 text-label-sm">
                <span className="text-secondary">{t.admin.disputeDamageAmount}: <strong className="text-on-background">{gel(dispute.damageAmount ?? 0)}</strong></span>
                <span className="text-secondary">{t.admin.disputeExpertFee}: <strong className="text-on-background">{gel(dispute.expertFee ?? 0)}</strong></span>
              </div>
              {dispute.expertNote && (
            ...
            <div className="flex flex-wrap gap-3 pt-1">
              {dispute.status === 'PENDING' && (
                <button onClick={assignExpert} disabled={assigning}
                  ...
                  {assigning
                    ? <><span className="material-symbols-outlined animate-spin text-[16px]">autorenew</span>{t.admin.disputeAssigning}</>
                    : <><span className="material-symbols-outlined text-[16px]">person_search</span>{t.admin.disputeAssignExpert}</>}
                </button>
              )}
          ...
          {resolveOpen && dispute.status !== 'RESOLVED' && (
            <div className="rounded-xl border border-outline-variant/50 bg-surface-container-low p-4 space-y-3">
              <p className="font-bold text-label-bold text-on-background">{t.admin.disputeEnterDetails}</p>
              <div className="grid sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold text-secondary uppercase tracking-wider block mb-1">{t.admin.disputeDamageAmount}</label>
                ...
                <div>
                  <label className="text-[11px] font-bold text-secondary uppercase tracking-wider block mb-1">{t.admin.disputeExpertFee}</label>
              ...
              <div>
                <label className="text-[11px] font-bold text-secondary uppercase tracking-wider block mb-1">{t.admin.disputeExpertNote}</label>
                <textarea value={expertNote} onChange={e => setExpertNote(e.target.value)}
                  placeholder={t.admin.disputeExpertNote}
                  rows={2}
              ...
              <div className="flex gap-3">
                ...
                <button onClick={resolveDispute} disabled={resolving}
                  ...
                  {resolving
                    ? <><span className="material-symbols-outlined animate-spin text-[16px]">autorenew</span>{t.admin.disputeSaving}</>
                    : <><span className="material-symbols-outlined text-[16px]">task_alt</span>{t.admin.disputeConfirmResolution}</>}
                </button>
              </div>
            </div>
          )}
  ...
  return (
    <div className="min-h-screen bg-surface pt-[120px] pb-20">
      <div className="mx-auto max-w-screen-lg px-4 md:px-8">
        ...
        <div className="mb-8">
          <h1 className="text-h1 font-extrabold text-on-background">{t.admin.disputeTitle}</h1>
          <p className="text-secondary text-label-sm mt-1">{t.admin.disputeSub}</p>
        </div>
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {(['ALL', 'PENDING', 'EXPERT_ASSIGNED', 'RESOLVED'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {f === 'ALL' ? t.admin.filterAll : { PENDING: t.admin.disputeStatusPending, EXPERT_ASSIGNED: t.admin.disputeStatusExpert, RESOLVED: t.admin.disputeStatusResolved }[f]}
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-20 text-center">
            ...
            <p className="font-bold text-label-bold text-secondary">{t.admin.disputeNone}</p>
          </div>
        ) : (
