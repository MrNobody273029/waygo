...
interface Props {
  ...
  hostProfile: { fullName: string; rating: number; reviewCount: number; phone: string | null } | null;
}
...
function HostContactCard({ phone, name }: { phone: string; name: string }) {
  return (
    <div className="rounded-2xl border border-outline-variant/40 bg-white shadow-card p-5">
      <div className="flex items-center gap-2 mb-4">
        <span className="material-symbols-outlined text-[18px] text-primary">call</span>
        <p className="font-extrabold text-label-bold text-on-background">Host Contact</p>
      </div>
      <p className="text-label-sm text-secondary mb-3 truncate">{name}</p>
      <a
        href={waLink(phone)}
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center justify-center gap-2 w-full rounded-xl py-3 font-bold text-label-bold text-white transition active:scale-95"
    ...
    <>
      <main className="min-h-screen bg-surface pt-[62px] md:pt-[120px] pb-20 md:pb-0">
        <div className="mx-auto max-w-screen-lg px-4 md:px-8 py-10">
          ...
          {/* Confirmation code — shown after host approves */}
          {booking.confirmationCode && ['pending', 'confirmed', 'return_review', 'completed'].includes(booking.status) && (
            <div className="mb-6 rounded-2xl bg-on-background px-6 py-5 text-center">
              <p className="text-[11px] font-bold tracking-[3px] uppercase text-secondary mb-3">
                <span className="material-symbols-outlined text-[13px] align-middle mr-1">key</span>
                {t.bookingDetail.pickupCode}
              </p>
              <p className="font-mono text-[40px] md:text-[52px] font-black text-white tracking-[10px] leading-none mb-3">
                {booking.confirmationCode}
              </p>
              <p className="text-[12px] text-secondary">
                {t.bookingDetail.pickupCodeNote}
              </p>
            </div>
          )}
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
            ...
            <div className="space-y-5">
              ...
              {/* Host contact card — visible after host approval */}
              {hostProfile?.phone && ['pending', 'confirmed', 'return_review', 'completed', 'disputed'].includes(booking.status) && (
                <HostContactCard phone={hostProfile.phone} name={hostProfile.fullName} />
              )}
            </div>
          </div>
        </div>
