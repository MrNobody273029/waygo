...
async function getOwnedCar(req: NextRequest, id: string) {
  const session = await getServerSession(authOptions);
  if (!session) return { error: NextResponse.json({ error: 'Unauthorized' }, { status: 401 }) };
  const userId = (session.user as any).id as string;
  const car = await prisma.car.findUnique({ where: { id } });
  if (!car) return { error: NextResponse.json({ error: 'Not found' }, { status: 404 }) };
  if (car.ownerId !== userId) return { error: NextResponse.json({ error: 'Forbidden' }, { status: 403 }) };
  return { car, userId };
}

export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  const result = await getOwnedCar(req, params.id);
  if ('error' in result) return result.error;

  if (result.car.listingStatus === 'PENDING') {
    return NextResponse.json({ error: 'Cannot edit a listing while it is under review' }, { status: 403 });
  }
  ...
  if (owner) {
    ...
    const html = adminNotificationLayout(
      `<p style="margin:0 0 8px;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1.5px;">&#128663; მანქანის განცხადება განახლდა</p>
      <p style="margin:0 0 24px;font-size:22px;font-weight:800;color:#0f172a;">ხელახლა გადასინჯვა საჭიროა</p>
      ...
      <p style="margin:0;color:#64748b;font-size:14px;line-height:1.7;">ჰოსტმა განაახლა მანქანის განცხადება. გადადი ადმინ პანელში, გაეცანი განახლებულ ფოტოებს და ტექ.პასპორტს, შემდეგ დაადასტურე ან უარყავი.</p>`,
      adminUrl,
    ...
    try {
      await resend.emails.send({
        ...
        to: ADMIN_EMAIL,
        subject: `განახლებული განცხადება: ${carLabel} — ${owner.fullName}`,
        html,
      });
    } catch (err) {
      console.error('Admin car resubmit notification email error:', err);
    }
  }

  return NextResponse.json({ carId: updated.id });
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (result.car.listingStatus === 'PENDING') {
    return NextResponse.json({ error: 'Cannot delete a listing while it is under review' }, { status: 403 });
  }
